...
- CLICK the green ```<>code``` button and DOWNLOAD as a zip file (shown in Downloads folder as ```Egtech-CE-Reforged-1.1.0.ZIP```)
...
- MOVE the 3 files ```'config', 'kubejs', 'mods'``` OUT of the ```Egtech-CE-Reforged-1.1.0.ZIP``` folder and INTO the ```.minecraft``` folder
